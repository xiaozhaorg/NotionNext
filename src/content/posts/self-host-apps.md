---
title: 自托管 20 个必备开源应用：告别云服务订阅，掌控自己的数据
pubDatetime: "2026-07-21T00:00:00.000Z"
description: 精选 20 个最值得自托管的开源应用，替代各类付费云服务，从笔记、网盘、影音到协作工具，掌控自己的数据。
author: 小吒
tags:
  - 免费工具
  - 开源
  - 教程
featured: false
draft: false
sourceUrl: "https://xiaozha.org/article/self-host-apps"
ogImage: "https://xiaozha.org/images/self-host-apps-cover.jpg?t=3a9c55d5-e9ea-816c-99b6-e4d25f79c8bd"
---

![image](https://images.unsplash.com/photo-1504639725590-34d0984388bd?w=1200&h=630&fit=crop&t=3aac55d5-e9ea-81ff-8660-df563aac375c&q=50&width=1080&fmt=webp&fm=webp)

云服务越来越贵，数据隐私越来越让人担忧。

这 20 个开源应用帮你自托管，替代各类付费服务。

一、笔记 & 文档
- AppFlowy - Notion 替代
- Outline - 团队 Wiki
- Affine - 笔记+白板

### 二、网盘 & 文件
- Nextcloud - Google Drive 替代
- FileBrowser - 轻量文件管理
- Seafile - 企业网盘

### 三、影音 & 媒体
- Jellyfin - Plex 替代
- Navidrome - Spotify 替代
- Immich - Google Photos 替代

### 四、密码 & 安全
- Bitwarden/Vaultwarden - 1Password 替代
- Authentik - 身份认证

### 五、协作 & 沟通
- Mattermost - Slack 替代
- Element - Discord 替代
- Bookstack - 团队文档

### 六、工具类
- Homepage - 个人导航页
- Uptime Kuma - 网站监控
- n8n - 工作流自动化
- Mealie - 菜谱管理
- Wallabag -稍后读
- Gitea - GitHub 替代

### 七、部署建议
- 使用 Docker Compose 统一管理
- Cloudflare Tunnel 暴露到公网
- 定期备份重要数据
- 反向代理 + HTTPS

[上一篇硅基流动（SiliconFlow）是什么？

一文读懂这个 AI 基础设施新星](/article/siliconflow-intro)[下一篇Rust vs Go 2026：

谁才是后端开发之王？

深度对比帮你选型](/article/rust-vs-go)

[下一篇Rust vs Go 2026：

谁才是后端开发之王？

深度对比帮你选型](/article/rust-vs-go)

- **作者:**[小吒博客](/about)
- **链接:**[https://xiaozha.org/article/self-host-apps](https://xiaozha.org/article/self-host-apps)
- **声明:**本文采用 CC BY-NC-SA 4.0 许可协议，转载请注明出处。

相关文章

[薅羊毛！

肖恩AI 免费大模型 API 中转站，注册即送 7000 额度（附接入教程）![image](https://xiaozha.org/images/shawn-ai-free-api-cover.jpg?t=3aec55d5-e9ea-81c8-929b-f8a61e29177f)](/article/shawn-ai-free-api)[Claude Code 实战指南：

终端里的 AI 编程代理怎么用？（2026 国内版）![image](https://xiaozha.org/images/claude-code-tutorial-cover.jpg?t=3adc55d5-e9ea-813d-a1bd-ef7ad19790cc)](/article/claude-code-tutorial)[Mineradio 音乐播放器教程（2026）：3D 立体歌词开源播放器，支持网易云 QQ 音乐同步![image](https://xiaozha.org/images/mineradio-tutorial-cover.jpg?t=3acc55d5-e9ea-8103-b388-c846fa950c37)](/article/mineradio-tutorial)[NextChat 部署指南：

Vercel 一键部署专属 AI 助手，支持 DeepSeek 等 16+ 大模型![image](https://xiaozha.org/images/nextchat-deploy-guide-cover.jpg?t=3a9c55d5-e9ea-81e1-86b5-f70a820365a8)](/article/nextchat-deploy-guide)[Zed 编辑器体验：

号称最快的代码编辑器，到底有多强？![image](https://xiaozha.org/images/zed-vs-vscode-cover.jpg?t=3a9c55d5-e9ea-8179-b3b3-c654b8779007)](/article/zed-vs-vscode)[Windows 包管理器 winget 完全指南：

告别手动下载安装软件![image](https://xiaozha.org/images/winget-windows-tools-cover.jpg?t=3a9c55d5-e9ea-8125-9a64-e68f8868196e)](/article/winget-windows-tools)
